...
מציג את הקלף הנוכחי ומאפשר לשלוט על הצבע של הטקסט
...
כניסה למצב מתקדם - במצב זה לפני כל תחילת משחק תופיע חלונית הגדרות שמאפשרת לקבוע מאיפה החפיסה ומה האסטרטגיה של לקיחת הקלף.
...
בתיקיית `src` תוכלו למצוא קובץ טייפים `types.ts` המכיל טייפים גלובליים לשימושכם. מומלץ להוסיף לשם טייפים נוספים לבחירתכם אך אין לשנות את הטייפים הנוכחיים שם. הקוד שלכם צריך להסתמך על הטייפים האלה שכן גם `ui@` מסתמך על הטייפים הללו.
...
3. תוודאו שאתם מחלקים נכון לתיקיות

4. תוודאו שאתם עושים export עבור פונקציות, משתנים, מחלקות או טייפים שאתם רוצים שהקובץ ייחצן

5. אין להתעסק עם ספריית UI או קובץ הHTML ללא אישור מהמפקדים

6. אתם מקבלים את הפרוייקט עם תרגיל מקדים - יש לעשות את התרגיל המקדים לפני שמתחילים את הפרוייקט
...
בצעו את התרגיל ב-`main.ts`. זוהי תרגולת הכנה לתרגיל משחק הקלפים. אל תשקיעו בחלק זה זמן רב היות ומטרתה היא התנסות בלבד. 
...
עבור מערך זה של שמות, כתבו קוד שאחרי שלוחצים על שני הכפתורים (סדר הלחיצה לא חשוב) השם יוחלף לשם הבא ברשימה.
...
אם המשתמש בוחר בחפיסה מהקונפיגורציה וקלף ראשון מהחפיסה: המשחק ימשיך כך שאם לוחצים על כפתור ה-`Higher` הניקוד יעלה ב-1, אך כאשר לוחצים על כפתור `Lower` ואז על כפתור `Higher` המשחק יתאפס. אם כפתור `LOWER` נלחץ פעמיים, המשחק יסתיים בהפסד.
...
על מנת להתחיל את התרגיל ברגל ימין, להלן הוראות עבור מימוש חלקי לפיו עליכם לעקוב, משם תצטרכו להמשיך את ה-design באופן עצמאי.
